...
| 007 deployment — AWS (Terraform IaC + CI/CD) | 🟨 **accepted + authored**: model artifact via S3 (FR-006, live-tested); full `infra/` Terraform (VPC/RDS/S3/ECS+ALB/EventBridge/Secrets/IAM+OIDC) + `deploy.yml` + `run_pipeline.py` — **reference IaC, not yet `terraform apply`d** (credentialed session) |
