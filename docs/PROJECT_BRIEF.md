...
| 011 agentic analysis (LangGraph + LangSmith) | 🟩 **built**: multi-step agent (`/agent/analyze|investigate|report`), 3 modes on one ReAct graph + tools; groundedness self-check (mode-scoped honest-framing); LangSmith tracing (env); AgentPanel in UI; live-verified (6-tool chains) |
| 007 deployment — AWS (Terraform IaC + CI/CD) | 🟨 **accepted (v1.2.0) + authored + validated**: model artifact via S3 (FR-006, live-tested); env-driven S3 client + `.env.example`; **dashboard = static export → S3 + CloudFront** (`/api/*`→ALB same-origin, static export **build-verified**); full `infra/` Terraform (VPC+endpoints/RDS/S3/ECS+ALB/CloudFront+**WAF**/**Route53+ACM domains**/EventBridge/Secrets/IAM+OIDC) + **docs/DEPLOYMENT.md** + `deploy.yml` + `run_pipeline.py` — **validated IaC (terraform validate passes both roots), not yet `terraform apply`d** (credentialed session) |
...
| 012 MWAA orchestration *(alt. to EventBridge→Fargate)* | 🟨 **accepted (alt track) + authored**: Airflow DAG (ingest→flag→dbt→train) + `infra/mwaa/` (MWAA env, DAG bucket, role, SG) — reference IaC, `terraform validate`-clean, **not applied** (~$350/mo idle) |
...
accuracy without adding insight. We deliberately do **not** *auto-select* (the set stays domain-chosen for interpretability), but we now
**measure** the choice (FR-011): every `make train` computes per-feature **mutual information**
(`mutual_info_regression` — the regression form of *information gain*, capturing non-linear dependence
correlation misses) + the winning model's **`feature_importances_`**, prints a ranked table, and saves
it to the model metadata. The live result is itself a finding: **`fertility_rate` carries the strongest
signal (MI 0.55) and `health_spend_pct_gdp` the weakest (MI 0.21)** — spending alone barely predicts
outcomes, which is precisely why the *value-for-money residual* is the product, not raw spend.
(Permutation importance, correlation/VIF pruning, and derived features like `log(GDP)` are natural
next steps.)
