...
**Status**: Accepted (2026-08-18) · amended **v1.1.0** (2026-08-21 — FR-011 feature-importance analysis).
...
- **FR-011 (v1.1.0)**: Training MUST compute **feature-importance evidence** so feature choice is
  *measured*, not only expert-judged: **mutual information** (`mutual_info_regression` — the regression
  form of information gain, captures non-linear dependence) and the winning model's **importances**
  (RandomForest `feature_importances_`). The per-feature scores are written to the model metadata and
  printed as a ranked table. This does not auto-select features (the set stays domain-chosen for
  interpretability) — it validates that choice with data.
